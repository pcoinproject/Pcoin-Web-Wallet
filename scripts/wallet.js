...
const LEDGER_ERRS = new Map([
  ...
  [25870, "Open the PCOIN app on your device"],
  [25873, "Open the PCOIN app on your device"],
  [57408, "Navigate to the PCOIN app on your device"],
  [27157, "Wrong app! Open the PCOIN app on your device"],
  [27266, "Wrong app! Open the PCOIN app on your device"],
  [27904, "Wrong app! Open the PCOIN app on your device"],
      ...
      if (await verifyMnemonic(privateImportValue)) {
        ...
        try {
          if (privateImportValue.startsWith("xprv")) {
            ...
            // Hide the 'new address' button, since non-HD wallets are essentially single-address PWW wallets
...
async function verifyMnemonic(strMnemonic = "", fPopupConfirm = true) {
  ...
  if (nWordCount >= 12 && nWordCount <= 24) {
    if (!bip39.validateMnemonic(strMnemonic)) {
      ...
      return fPopupConfirm && await confirmPopup({ title: "Unexpected Seed Phrase", html: "The seed phrase is either invalid, or was not generated by PWW.<br>Do you still want to proceed?"});
    } else {
